...
```bash
polydep check <expected_graph_file> [--root <path>]
```

The expected graph file is a Mermaid file, typically generated by `polydep graph > file.mermaid`. Strict comparison — unexpected edges (boundary violations), missing edges (stale graph), and unknown bricks all cause failure.
